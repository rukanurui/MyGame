...
ParticleManager2d* ParticleManager2d::Create(ID3D12Device* device, Camera* camera)
{
	// 3Dオブジェクトのインスタンスを生成
	ParticleManager2d* partMan = new ParticleManager2d(device, camera);
	if (partMan == nullptr) {
		return nullptr;
	}
...
void ParticleManager2d::Draw(ID3D12GraphicsCommandList* cmdList)
{
...
void ParticleManager2d::Add(int life, XMFLOAT3 position, XMFLOAT3 velocity, XMFLOAT3 accel, float start_scale, float end_scale)
{
	// リストに要素を追加
...
void ParticleManager2d::LoadTexture()
{
	HRESULT result = S_FALSE;
	...
	result = LoadFromWICFile(
		L"Resources/enemy.jpg", WIC_FLAGS_NONE,
		&metadata, scratchImg);
	if (FAILED(result)) {
		assert(0);
	}
...
void ParticleManager2d::CreateModel()
{
	HRESULT result = S_FALSE;
...
ParticleManager2d::ParticleManager2d(ID3D12Device* device, Camera* camera)
{
	this->device = device;
	this->camera = camera;
}